...
export const caminhoes = mysqlTable("caminhoes", {
  ...
  ano: int("ano").notNull(),
  capacidadeToneladas: decimal("capacidadeToneladas", { precision: 10, scale: 2 }),
  ...
  status: mysqlEnum("status", ["operacional", "manutencao", "inativo"]).default("operacional").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
...
export const clientes = mysqlTable("clientes", {
  ...
  estado: varchar("estado", { length: 2 }),
  cep: varchar("cep", { length: 10 }),
  tipoCliente: mysqlEnum("tipoCliente", ["pf", "pj"]).notNull(),
  limiteCreditoReais: decimal("limiteCreditoReais", { precision: 12, scale: 2 }).default("0"),
  saldoDevedorReais: decimal("saldoDevedorReais", { precision: 12, scale: 2 }).default("0"),
...
// Caçambas
export const cacambas = mysqlTable("cacambas", {
  id: int("id").autoincrement().primaryKey(),
  ...
  statusDisponibilidade: mysqlEnum("statusDisponibilidade", ["disponivel", "alugada", "manutencao", "inativa"]).default("disponivel").notNull(),
  dataAquisicao: timestamp("dataAquisicao"),
  valorAquisicaoReais: decimal("valorAquisicaoReais", { precision: 12, scale: 2 }),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
...
export const pedidos = mysqlTable("pedidos", {
  ...
  valorLocacaoReais: decimal("valorLocacaoReais", { precision: 12, scale: 2 }).notNull(),
  observacoes: text("observacoes"),
  status: mysqlEnum("status", ["pendente", "em_andamento", "entregue", "retirado", "faturado", "cancelado"]).default("pendente").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
...
export const transacoes = mysqlTable("transacoes", {
  ...
  valorReais: decimal("valorReais", { precision: 12, scale: 2 }).notNull(),
...
export const manutencoes = mysqlTable("manutencoes", {
  id: int("id").autoincrement().primaryKey(),
  tipo: mysqlEnum("tipo", ["caminhao", "cacamba"]).notNull(),
  ...
  custoReais: decimal("custoReais", { precision: 12, scale: 2 }),
